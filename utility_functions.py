...
def same_files_in_folder(folder1, folder2, shallow = False): # checks if files in folder and all subfolders are the same
    ...
    if shallow and comparison.diff_files: # differences in size or modification time
        ...
        for file in comparison.common_files:
            if not filecmp.cmp(Path(folder1)/file, Path(folder2)/file, shallow = False):
                return False
...
def copy_data(origin_folder: Path, destination_folder: Path): # copies data from local to final storage, then deletes local copy
    if not is_folder_with_files(origin_folder): # check if data exists locally
    ...
    if same_files_in_folder(origin_folder, destination_folder): # check if everything has been copied correctly
        ...
        print(f"✓ Copied data to {destination_folder} and removed local copy.\n")
    else:
...
def check_free_space(folder: Path, required_space): # checks if there is enough free space in given folder, required_space in GB
    usage = shutil.disk_usage(folder.anchor)
    ...
    if free_space_gb < required_space:
        raise RuntimeError(f"Not enough free space in {folder}. Required: {required_space} GB, Available: {free_space_gb:.2f} GB.\n"
                           "If you are using neuropixels_data.run_preprocessing() on many files, consider using neuropixels_data.run_processing_pipeline() with delete_preprocessing_data = True instead.\n")
...
def get_folder_size_GB(folder_path: Path):
    total_size_folder = sum(file.stat().st_size for file in Path(folder_path).rglob('*') if file.is_file())
    return total_size_folder / (1024 ** 3)


def get_size_of_folders(folders: list):
    total_size_GB = 0
